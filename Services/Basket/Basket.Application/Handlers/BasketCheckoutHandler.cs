...
{
    // user click to checkout -> RabitMQ sends event to order for create -> delete basket 
    public class BasketCheckoutHandler : IRequestHandler<BasketCheckoutCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(BasketCheckoutCommand request, CancellationToken cancellationToken)
        {
            ...
            // delete the basket
            await _mediator.Send(new DeleteBasketByUserNameCommand(basket.UserName),cancellationToken);
            return Unit.Value;
        } 
    }
}
